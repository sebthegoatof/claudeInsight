...
async function loadTree() {
  ...
  error.value = null;
  try {
    const response = await assetsApi.getCommandTree();
...
async function loadFile(path: string) {
  ...
  try {
    const response = await assetsApi.getCommandFile(path);
...
async function saveFile() {
  ...
  try {
    await assetsApi.saveCommandFile(selectedPath.value, fileContent.value);
...
async function createNode() {
  ...
  creating.value = true;
  try {
    const response = await assetsApi.createCommandNode(
...
async function deleteNode() {
  deleting.value = true;
  try {
    const response = await assetsApi.deleteCommandNode(deletePath.value);
    if (response.success) {
    ...
    <div class="w-72 flex-shrink-0 border-r border-border flex flex-col bg-card/30">
      <div class="flex items-center justify-between px-3 py-2.5 border-b border-border">
        <div class="flex items-center gap-2">
          ...
          <span class="text-sm font-medium">自定义命令</span>
        </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      ...
      <div v-else class="flex-1 flex items-center justify-center">
        <div class="text-center">
          ...
          <p class="text-sm text-muted-foreground">选择一个自定义命令进行编辑</p>
          <p class="text-xs text-muted-foreground mt-1">或点击 + 创建新的命令</p>
        </div>
      </div>
    </div>
