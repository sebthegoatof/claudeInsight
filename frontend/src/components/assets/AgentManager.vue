...
async function loadTree() {
  ...
  error.value = null;
  try {
    const response = await assetsApi.getAgentTree();
...
async function loadFile(path: string) {
  ...
  try {
    const response = await assetsApi.getAgentFile(path);
...
async function saveFile() {
  ...
  try {
    await assetsApi.saveAgentFile(selectedPath.value, fileContent.value);
...
async function createNode() {
  ...
  creating.value = true;
  try {
    const response = await assetsApi.createAgentNode(
    ...
    if (response.success) {
      ...
      await loadTree();
      if (createParentPath.value) expandedPaths.value.add(createParentPath.value);
...
async function deleteNode() {
  deleting.value = true;
  try {
    const response = await assetsApi.deleteAgentNode(deletePath.value);
    if (response.success) {
...
<template>
  <div class="h-full flex overflow-hidden" @keydown="handleKeydown" tabindex="0">
    <!-- 左侧：文件树 -->
    <div class="w-72 flex-shrink-0 border-r border-border flex flex-col bg-card/30">
      <div class="flex items-center justify-between px-3 py-2.5 border-b border-border">
        <div class="flex items-center gap-2">
          <Bot class="w-4 h-4 text-primary" />
          <span class="text-sm font-medium">Agent 模板</span>
        </div>
        <div class="flex items-center gap-1">
          <button @click="openCreateDialog('', 'directory')" class="p-1 rounded hover:bg-muted transition-colors" title="新建文件夹">
            <FolderPlus class="w-4 h-4 text-muted-foreground" />
          </button>
          <button @click="openCreateDialog('', 'file')" class="p-1 rounded hover:bg-muted transition-colors" title="新建文件">
            <FilePlus class="w-4 h-4 text-muted-foreground" />
          </button>
          <button @click="refresh" :disabled="loading" class="p-1 rounded hover:bg-muted transition-colors" title="刷新">
            <RefreshCw :class="['w-4 h-4 text-muted-foreground', { 'animate-spin': loading }]" />
          </button>
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-1">
        ...
        <div v-else-if="!treeData?.children?.length" class="p-4 text-center text-sm text-muted-foreground">
          ...
          <p>暂无 Agent 模板</p>
          <button @click="openCreateDialog('', 'file')" class="mt-2 text-primary hover:underline text-xs">
            创建第一个 Agent
          </button>
        </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      ...
      <div v-if="selectedPath" class="flex-1 overflow-hidden">
        ...
        <textarea
          v-else
          v-model="fileContent"
          class="w-full h-full p-4 bg-transparent text-sm font-mono resize-none focus:outline-none"
          placeholder="编写 Agent 指令..."
          spellcheck="false"
        />
      </div>
      ...
      <div v-else class="flex-1 flex items-center justify-center">
        <div class="text-center">
          ...
          <p class="text-sm text-muted-foreground">选择一个 Agent 模板进行编辑</p>
          <p class="text-xs text-muted-foreground mt-1">或点击 + 创建新的 Agent</p>
        </div>
      </div>
    </div>
      ...
      <div v-if="showCreateDialog" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="showCreateDialog = false">
        <div class="bg-card border border-border rounded-lg shadow-lg w-80 p-4">
          ...
          <div class="flex justify-end gap-2 mt-4">
            <button @click="showCreateDialog = false" class="px-3 py-1.5 text-sm rounded-md border border-border hover:bg-muted transition-colors">
              取消
            </button>
            <button
              @click="createNode"
      ...
      <div v-if="showDeleteConfirm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="showDeleteConfirm = false">
        <div class="bg-card border border-border rounded-lg shadow-lg w-80 p-4">
          ...
          <div class="flex justify-end gap-2">
            <button @click="showDeleteConfirm = false" class="px-3 py-1.5 text-sm rounded-md border border-border hover:bg-muted transition-colors">
              取消
            </button>
            <button
              @click="deleteNode"
